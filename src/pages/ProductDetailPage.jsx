...
const ProductDetailPage = ({
        ...
        if (data.sellerId) {
          try {
            const ratingRes = await fetch(
              `${import.meta.env.VITE_API_URL}/api/ratings/user/${data.sellerId}`,
            );
            if (ratingRes.ok) {
  ...
  useEffect(() => {
    const checkFavorite = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/favorites/check/${productId}`,
          {
  ...
  useEffect(() => {
    const checkPurchaseStatus = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/transactions/product/${productId}/status`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const handlePurchaseRequest = async () => {
    ...
    if (!token) {
      showToast(
        "error",
        t("post.login_required", { defaultValue: "請先登入" }),
      );
    ...
    if (product?.sellerId === userId) {
      showToast(
        "error",
        t("product.own_product_error", { defaultValue: "不能購買自己的商品" }),
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          "success",
          t("product.purchase_request_sent", {
            defaultValue: "已發送購買請求給賣家",
          }),
        );
      } else {
        ...
        showToast(
          "error",
          err.message ||
            t("product.purchase_request_failed", {
              defaultValue: "發送購買請求失敗",
            }),
        );
      }
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      ...
      showToast(
        "success",
        t("product.share_link_copied", { defaultValue: "連結已複製到剪貼簿" }),
      );
    } catch (err) {
      console.error("Failed to copy:", err);
      showToast(
        "error",
        t("product.share_failed", { defaultValue: "複製失敗，請稍後再試" }),
      );
    }
  };
  ...
  const handleReport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        showToast(
          "success",
          t("product.report_sent", {
            defaultValue: "檢舉已送出，我們會盡快處理",
          }),
        );
      ...
      showToast(
        "error",
        t("post.error", { defaultValue: "系統錯誤，請稍後再試" }),
      );
    } finally {
      setSubmitting(false);
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          "error",
          err.message ||
            t("auth.operation_failed", { defaultValue: "無法開啟對話" }),
        );
      }
    } catch (error) {
  ...
  if (loading)
    return (
      <div className="p-10 text-center text-pine-600">
        {t("home.loading", { defaultValue: "載入中..." })}
      </div>
    );
  if (error || !product)
    return (
      <div className="p-10 text-center text-pine-600">
        {t("product.detail_not_found", { defaultValue: "找不到商品" })}
      </div>
    );
  ...
  return (
    <div className="h-full overflow-hidden bg-transparent">
      ...
      {toast && (
        <div
          className={`fixed top-6 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-2xl shadow-lg flex items-center gap-3 ${
            toast.type === "success"
      ...
      <div className="max-w-2xl mx-auto h-full flex flex-col lg:items-center">
        <div className="bg-white rounded-t-3xl shadow-sm flex-1 flex flex-col min-h-0 overflow-hidden lg:w-full lg:origin-top lg:scale-[0.95]">
          ...
          <div className="min-h-[35%] max-h-[46%] bg-cream-50 flex flex-col overflow-hidden flex-shrink-0 relative">
            ...
            <button
              onClick={() => {
                if (productBackPage) {
                  if (
                    productBackPage.type === "chat" &&
                    productBackPage.chatId
                  ) {
                    if (onNavigateToChat) {
                    ...
                    productBackPage.type === "seller" &&
                    productBackPage.sellerId
                  ) {
                  ...
                  <div className="h-full flex gap-1.5 md:gap-5 px-2 py-1.5">
                    {images.map((img, idx) => (
                      <button
                        key={idx}
                        onClick={() => setSelectedImageIndex(idx)}
                        className={`h-full aspect-square rounded-lg overflow-hidden flex-shrink-0 transition-all ${
                          idx === selectedImageIndex
          ...
          <div className="flex-1 p-4 md:p-6 pb-20 flex flex-col min-h-0 overflow-y-auto">
            ...
            <div className="flex-shrink-0 space-y-2">
              <div>
                <div className="flex items-start justify-between mb-1">
                  <h1 className="text-2xl md:text-3xl font-light text-pine-900 tracking-wide truncate">
                    {product.title}
                  </h1>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      title="分享商品"
                    >
                      <Share2 size={20} className="text-pine-400" />
                    </button>
                    <button
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-pine-400 font-light">
                  {product.negotiable
                    ? t("product.negotiable", { defaultValue: "可議價" })
                    : t("product.not_negotiable", { defaultValue: "不議價" })}
                </span>
              </div>
              {product.deliveryMethod?.includes("寄送") && (
                <span className="inline-block text-xs text-amber-700 bg-amber-50 px-2 py-1 rounded-full border border-amber-200 w-fit">
                  {t("product.shippable", { defaultValue: "可寄送" })}
                </span>
              )}
              ...
              <div className="border-t border-pine-100 pt-3 flex items-center justify-between">
                ...
                >
                  ...
                  <div>
                    <span className="text-pine-800 font-medium block text-sm">
                      {product.seller?.name ||
                        t("profile.unknown_user", { defaultValue: "未知賣家" })}
                    </span>
                    <span className="text-xs text-pine-500">
                      {product.seller?.department
                        ? t(`departments.${product.seller.department}`, {
                            defaultValue: product.seller.department,
                          })
                        : ""}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-medium text-pine-700 text-sm">
                    {sellerRating.averageScore.toFixed(1)}
                  </span>
                  <span className="text-pine-400 text-xs">
            ...
            <div className="flex-1 min-h-0 border-t border-pine-100 mt-3 pt-3 flex flex-col max-h-56">
              <h3 className="text-sm font-medium text-pine-600 mb-2 flex-shrink-0">
                {t("product.about_item", { defaultValue: "關於這件物品" })}
              </h3>
            ...
            <div className="flex-shrink-0 pt-3 space-y-3">
                ...
                <div className="border-t border-pine-100 pt-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(product.location
                      ...
                      <span
                        key={`${point}-${index}`}
                        className="bg-forest-50 text-forest-700 px-2 py-1 rounded-full text-xs border border-forest-100"
                      >
                        {point}
                      </span>
                    ))}
              ...
              <div className="flex gap-2 pt-6">
                ...
                <button
                  ...
                  disabled={
                    hasPendingPurchase || purchaseLoading || product?.reserved
                  }
                  className={`flex-1 py-3 rounded-2xl font-medium transition shadow-md hover:shadow-lg transform active:scale-[0.98] text-sm flex items-center justify-center gap-2 ${
                    hasPendingPurchase || product?.reserved
                ...
                >
                  <ShoppingBag size={16} />
                  {purchaseLoading
                    ? t("product.status_pending", { defaultValue: "發送中..." })
                    : hasPendingPurchase
                      ? t("product.requested", { defaultValue: "已發送請求" })
                      : product?.reserved
                        ? t("product.reserved", { defaultValue: "已保留" })
                        : t("product.confirm_purchase", {
                            defaultValue: "確認購買",
                          })}
                </button>
                <button
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="p-6 border-b border-pine-100">
              <h3 className="text-xl font-medium text-pine-900">
                {t("product.report_product", { defaultValue: "檢舉此商品" })}
              </h3>
              <p className="text-sm text-pine-500 mt-1">
                {t("product.report_reason_title", {
                  defaultValue: "請選擇檢舉原因",
                })}
              </p>
            </div>
            ...
            <div className="p-6 border-t border-pine-100 flex gap-3">
              ...
              >
                {submitting
                  ? t("product.status_pending", { defaultValue: "送出中..." })
                  : t("product.report_product", { defaultValue: "送出檢舉" })}
              </button>
            </div>
          </div>
        </div>
